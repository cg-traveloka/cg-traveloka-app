...
function HotelInfo(params) {
    ...
    useEffect(() => {
        axios.get("/api/rooms", {
            ...
            .then(result => {
                console.log(result.data.availableRooms);
                setRooms(result.data.availableRooms);
                setHotel(result.data.availableRooms[0].hotel);
                // console.log(result.data.availableRooms);
                let arr = [];
                for (let index = 0; index < result.data.availableRooms[0].hotel.hotelStar; index++) {
                    arr.push(index);
                }
                setStars(arr);
            })
    ...
    useEffect(
        () => {
            axios.get("/api/images", { params: { hotelId: id } })
                .then(result => {
    ...
    useEffect(
        () => {
            axios.get("/api/utilities", { params: { hotelId: id } })
                .then(result => {
        ...
        <div className="hotelInfo">
            <div className="container">
                <div className="group1">
                    <div className="left">
                        ...
                        <div className="hotel-starGroup">
                            <span className="text">Khách sạn</span> {stars.map((value, index) => <span key={index}><HotelStarSmall /></span>)}
                        </div>
                        <div className="location"> <LocationIconBlack /><p>{hotel.address}</p></div>
                    </div>
                    <div className="right">
                        <div className="text">Giá/phòng/đêm từ</div>
                        <div className="price">{`${hotel.minSellPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} VND`}</div>
                ...
                <div className="group3">
                    <div className="left">
                        <div className="row1"><h3>Giới thiệu cơ sở lưu trú</h3> <div className="loadmore">Xem thêm</div></div>
                        <div className="row2">
                            <p>
                                {hotel.description}
                            </p>
                    ...
                    <div className="right">
                        <div className="facility">
                            <div className="row1"><h3>Tiện ích chính</h3></div>
                            <div className="row2">
                                {utilities.map(util => <div key={"div" + util.id} className="group"><TravelokaIcon /> <span key={"span" + util.id} >{util.hotelUtility.name}</span></div>)}
                            </div>
                        </div>
                                ...
                                <div className="comment-tag">
                                    <div className="comment-tag-row1">
                                        <div className="comment-tag-left">
                                            <span className="travelIcon"><TravelokaIcon /></span><span className="ratePoint">10.0</span><span className="staticPoint">/ 10</span>
                                        </div>
                                    ...
                                    <div className="comment-tag-row2">
                                        <p>Khách sạn đối diện khúc biển cát rất êm, không hề có đá. Lần đầu tắm biển mà cảm thấy không sợ bị xước da chảy máu. Khúc biển đối diện cũng không đông đúc. Dậy sớm ăn sáng rồi tắm luôn là hết sẩy. Nhân viên xách hành lý lịch sự nhã nhặn. Nhìn rất hiền. Ra vô luôn có lễ tân cúi chào. Anh quản lý làm nhân viên check in siêu nhanh. Có welcome drink nữa. Nói chung là ổn trong tầm giá. View biển khá đẹp.</p>
                                    </div>
                                </div>
                                <div className="comment-tag">
                                    <div className="comment-tag-row1">
                ...
                <div id="rooms" className="group4">
                    <div className="group4-title">Những phòng còn trống tại khách sạn</div>
                    {rooms.map((room) => <RoomCard room={room} />)}
                </div>
            </div>
            <ImageGallery showIndex={true} ref={gallaryRef} className="gallary" items={gallery} />
        </div>
    )
}
