    ...
    return (
        <div onClick={handleChooseHotel} className="hotelCard">
            ...
            <div className="hotelInfo">
                <div className="row1">
                    <div className="hotelName"><p>{hotel.hotelName}</p></div>
                    <div className="hotelRate">
                        <span className="traveloka-logo"><TravelokaIcon /></span>
                        <span className="points">{hotel.averagePoint}</span>
                        <span className="numbers">({hotel.hotelBookedNumbers.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")})</span>
                    </div>
                </div>
                <div className="row2">
                    <HotelIcon1 /> <span>Khách sạn</span> {stars.map((value) => <span key={value} ><HotelStarSmall /></span>)}
                </div>
                <div className="row3">
            ...
            <div className="hotelPrice">
                <div className="row12">
                    <div className="row1">
                        <span>{`${originPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} VND`}</span>
                    </div>
                    <div className="row2">
                        <span>{`${sellPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} VND`}</span>
                    </div>
                </div>
                <div className="row3">
