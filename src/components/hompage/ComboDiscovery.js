import React from "react";

function ComboDiscovery() {
  return (
    <div className="container w-4/5  mx-auto mb-20">
      <div className="my-5">
        <span>
          <i
            class="fa-solid fa-rectangle-ad fa-2xl mx-5"
            style={{ color: "#0c2a5f" }}
          ></i>
        </span>
        <span className="font-bold text-xl">Rẻ hơn với combo</span>
      </div>
      <div className="flex mx-auto mt-7">
        <div class="w-max overflow-hidden bg-white rounded-lg shadow-md ml-4  transition duration-300 ease-in-out hover:-translate-y-2 hover:scale-105">
          <div class="relative">
            <img
              class="w-64 h-full object-cover"
              src="https://images.unsplash.com/photo-1555979864-7a8f9b4fddf8?q=80&w=2371&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt="Image"
            />
            <div class="absolute top-0 right-0">
              <div class="w-20 h-7 absolute top-2 right-0">
                <div class="h-full w-full bg-rose-600 text-white text-center text-sm leading-7 font-semibold transform">
                  -20% OFF
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-between w-64">
            <div className="p-4  relative">
              <h3 className="text-lg font-semibold mb-1">Đà Nẵng</h3>
              <p className="text-gray-700 text-sm line-through">
                VNĐ 1,000,000
              </p>
              <p className="text-sm text-rose-500">VNĐ 800,000</p>
            </div>
            <div className="my-auto">
              <img
                src="https://i.pinimg.com/564x/7a/ec/17/7aec17946661a88378269d0b642b61f3.jpg"
                alt="Vietnam Airlines Logo"
                width={50}
                className="mx-6"
              />
            </div>
          </div>
        </div>
        <div class="w-max overflow-hidden bg-white rounded-lg shadow-md ml-4  transition duration-300 ease-in-out hover:-translate-y-2 hover:scale-105">
          <div class="relative">
            <img
              class="w-64 h-full object-cover"
              src="https://images.unsplash.com/photo-1557750255-c76072a7aad1?q=80&w=2370&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt="Image"
            />
            <div class="absolute top-0 right-0">
              <div class="w-20 h-7 absolute top-2 right-0">
                <div class="h-full w-full bg-rose-600 text-white text-center text-sm leading-7 font-semibold transform">
                  -10% OFF
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-between w-64">
            <div className="p-4  relative">
              <h3 className="text-lg font-semibold mb-1 ">Ninh Bình</h3>
              <p className="text-gray-700 text-sm line-through">
                VNĐ 1,000,000
              </p>
              <p className="text-sm text-rose-500">VNĐ 900,000</p>
            </div>
            <div className="my-auto">
              <img
                src="https://upload.wikimedia.org/wikipedia/commons/1/19/VietJet_Air_logo.svg"
                alt="Vietject Airlines Logo"
                width={80}
                className="mx-6"
              />
            </div>
          </div>
        </div>
        <div class="w-max overflow-hidden bg-white rounded-lg shadow-md ml-4  transition duration-300 ease-in-out hover:-translate-y-2 hover:scale-105">
          <div class="relative">
            <img
              class="w-64 h-full object-cover"
              src="https://images.unsplash.com/photo-1517420128935-affdd8d90ca0?q=80&w=2370&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt="Image"
            />
            <div class="absolute top-0 right-0">
              <div class="w-20 h-7 absolute top-2 right-0">
                <div class="h-full w-full bg-rose-600 text-white text-center text-sm leading-7 font-semibold transform">
                  -20% OFF
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-between w-64">
            <div className="p-4  relative">
              <h3 className="text-lg font-semibold mb-1 ">Cần Thơ</h3>
              <p className="text-gray-700 text-sm line-through">
                VNĐ 1,000,000
              </p>
              <p className=" text-sm text-rose-500">VNĐ 800,000</p>
            </div>
            <div className="my-auto">
              <img
                src="https://i.pinimg.com/564x/7a/ec/17/7aec17946661a88378269d0b642b61f3.jpg"
                alt="Vietnam Airlines Logo"
                width={50}
                className="mx-6"
              />
            </div>
          </div>
        </div>
        <div class="w-max overflow-hidden bg-white rounded-lg shadow-md ml-4  transition duration-300 ease-in-out hover:-translate-y-2 hover:scale-105">
          <div class="relative">
            <img
              class="w-64 h-40 object-cover"
              src="https://images.unsplash.com/photo-1514864151880-d1bef4892f29?q=80&w=2574&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt="Image"
            />
            <div class="absolute top-0 right-0">
              <div class="w-20 h-7 absolute top-2 right-0">
                <div class="h-full w-full bg-rose-600 text-white text-center text-sm leading-7 font-semibold transform">
                  -20% OFF
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-between w-64">
            <div className="p-4 relative">
              <h3 className="text-lg font-semibold mb-1 ">Quảng Nam</h3>
              <p className="text-gray-700 text-sm line-through">
                VNĐ 1,000,000
              </p>
              <p className=" text-sm text-rose-500">VNĐ 800,000</p>
            </div>
            <div className="my-auto">
              <img
                src="data:image/png;base64,"
                alt="Bamboo Airlines Logo"
                width={80}
                className="mx-6"
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default ComboDiscovery;
